...
当前源码树为尚未发布的 `v0.4.0` 候选版。该候选版新增一个受限、按需读取的 MCP 工作流契约
Resource，以及面向明确确认的 deep 目标的可选 Wave 模式。Wave 模式新增三个用于计划修订和
工作包证据的确定性工具，同时保持既有 direct quick/standard 路径不变，且不新增 MCP Prompt
模板。
...
失败与通过、落地和卸载。`v0.4.0` 候选版也已通过 ZCode 3.3.6 的计划展示、确认与认领、恢复、
