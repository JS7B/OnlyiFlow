...
- `v0.4.0` — 增加可选的 deep-only Wave 工作流、版本化工作包计划、三个 Wave 工具，以及受限的
  `onlyiflow://contract/concise` MCP Resource。该版本为当前正式版本。

当前源码树是 unreleased `0.5.0` 开发候选版，新增适用于 `standard` 工作的可选 Wave 模式，以及
经所有者确认的 `flow_close` 终态转换。在另行完成发布授权与验收前，`v0.4.0` 仍是当前 GitHub
正式版本。
...
当前 unreleased `0.5.0` 源码暴露十二个确定性 MCP 工具。`v0.4.0` 正式版包含前十一个工具，
`flow_close` 是候选版新增工具。
