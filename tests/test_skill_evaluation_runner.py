...
class SkillEvaluationRunnerTests(unittest.TestCase):
    def test_host_runner_exposes_the_current_exact_tool_inventory(self) -> None:
        self.assertEqual(
            TOOLS,
            (
                "project_status",
        ...
        self.assertEqual(len(CLAUDE_TOOLS), len(TOOLS))
    ...
    def test_project_setups_create_the_expected_preconditions(self) -> None:
        expected = {
            ...
            "wave_draft": (True, "draft", 0, 0),
            "wave_implementing": (True, "implementing", 1, 0),
        }
    ...
    def test_wave_proposal_requires_status_complete_plan_and_zero_mutation(
        self,
    ) -> None:
        snapshot = self.wave_snapshot(state="draft", specs=0, revision=0, packages=0)
        process = self.success_with_output(
            ["project_status"],
            "This is a deep + Wave goal. Complete plan: Goal; invariants; non-goals; "
            "packages P and Q; dependency DAG P then Q; Wave 0 and Wave 1; disjoint "
            "scopes; acceptance; authority boundaries. Please confirm the complete plan.",
        )
    ...
    def test_wave_plan_confirmation_requires_exact_persist_and_claim_sequence(
        self,
    ) -> None:
        before = self.wave_snapshot(state="draft", specs=0, revision=0, packages=0)
        after = self.wave_snapshot(
            state="implementing", specs=1, revision=1, packages=2
        )
        ...
        status, reasons, _ = evaluate_case(
            ...
            before=before,
            after=after,
            process=self.success_with_tools(
    ...
    def test_wave_incomplete_check_rejects_gate_run_even_without_state_mutation(
        self,
    ) -> None:
        snapshot = self.wave_snapshot(
            state="implementing", specs=1, revision=1, packages=2
        )
        ...
        status, reasons, _ = evaluate_case(
            ...
            before=snapshot,
            after=snapshot,
            process=self.success_with_tools("project_status", "gate_run"),
        )

        self.assertEqual(status, "failed")
        self.assertIn("unexpected_onlyiflow_tool_sequence", reasons)
        self.assertIn("wave_gate_called_before_packages_complete", reasons)

    def test_direct_quick_and_standard_sequences_remain_unchanged(self) -> None:
        evaluations = json.loads(
            (Path(__file__).parent / "fixtures/skill_evaluations.json").read_text(
                encoding="utf-8"
            )
        )
        direct = {case["id"]: case for case in evaluations["explicit"]}

        self.assertEqual(
            direct["quick-start"]["expected_tools"],
            ["project_status", "flow_start"],
        )
        self.assertEqual(
            direct["standard-start"]["expected_tools"],
            ["project_status", "flow_start", "spec_submit", "flow_claim"],
        )
    ...
    def success_with_output(self, tools: list[str], output: str) -> ProcessResult:
        tool_output = self.success_with_tools(*tools).stdout
    ...
    def wave_snapshot(
        self, *, state: str, specs: int, revision: int, packages: int
    ) -> dict:
        return {
            "managed": True,
            "flows": [{"risk": "deep", "state": state, "mode": "wave"}],
            "specs": specs,
            "gate_runs": 0,
            "events": 1,
